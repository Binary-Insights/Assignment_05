...
def update_payload_node(state: PayloadWorkflowState) -> PayloadWorkflowState:
    ...
    try:
        ...
        logger.info(
            ...
            f"filled_fields={len(metadata['filled_fields'])} "
            f"ambiguous_fields={len(metadata['ambiguous_fields'])} "
            f"unfilled_nulls={len(metadata['unfilled_nulls'])}"
        )
        ...
        return {
            **state,
            ...
            "update_result": {
                "status": "success",
                "filled_fields": metadata["filled_fields"],
                "ambiguous_fields": metadata["ambiguous_fields"],
                "unfilled_nulls": metadata["unfilled_nulls"],
                "field_confidence_map": metadata.get("field_confidence_map", {}),
                "provenance_updates": metadata.get("provenance_updates", {}),
            },
            "status": "completed",
        }
    except Exception as e:
...
def main():
    ...
    try:
        ...
        if final_state.get('update_result'):
            ...
            filled = len(update.get('filled_fields', []))
            unfilled = len(update.get('unfilled_nulls', []))
            ambiguous = len(update.get('ambiguous_fields', []))
            
            if filled > 0:
                print(f"   Filled: {filled} fields")
            if ambiguous > 0:
                print(f"   Ambiguous: {ambiguous} fields")
            print(f"   Unfilled: {unfilled} nulls")
